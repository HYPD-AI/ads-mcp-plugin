---
description: Full Google Ads account audit — structure, settings, tracking and performance, ranked by impact
argument-hint: "[account] [time period]"
---
Run a HYPD account audit for $ARGUMENTS.
...
3. Call `prompt_templates_run` with key `standard-account-audit` and follow the returned prompt's [input] directives — $ARGUMENTS may already answer some of them; ask only for what is still missing.

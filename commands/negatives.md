---
description: Mine search terms for negative keyword candidates in a Google Ads account
argument-hint: "[account] [campaign]"
---
Identify negative keyword opportunities for $ARGUMENTS.
...
3. Call `prompt_templates_run` with key `identify-negative-keywords` and follow the returned prompt's [input] directives — $ARGUMENTS may already answer some of them; ask only for what is still missing.
